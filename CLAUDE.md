...
**Next in queue:**
1. Settings screen
2. Script run-history **write side** — `script_log_write`, plus wiring it into `use-script-remote.ts` so a real terminal run actually produces a new log file instead of relying on the hand-seeded mock files. Must write into the user-configured `logsDirectoryPath` (see the "Logs directory decoupled" bullet above), not assume `<scripts_dir>/outputs/` — see `spec-backend.md` § "Script Run History"
