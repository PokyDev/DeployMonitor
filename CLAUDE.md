...
- Script run-history **read side** (2026-06-24): `script_log_list`/`script_log_get` (`script_log_service.rs`) read one-JSON-file-per-execution from a folder; the Historial view (`history.tsx`) now consumes them via `use-script-history.ts` instead of the old hardcoded `use-mock-history.ts` (removed). 6 hand-seeded `.json` files were originally seeded under the scripts directory's `outputs/` folder so the view had real data to render. See `spec-backend.md` § "Script Run History".
- Logs directory decoupled from the scripts directory (2026-06-24): Historial now has its own directory picker (locked path field + native folder picker, shared component `directory-path-field.tsx`, also used by Scripts) instead of implicitly reading `<scripts_dir>/outputs/`. Persisted independently as `logsDirectoryPath` in `history-settings.json` via `use-script-history.ts`, with a one-time migration default to `<scripts_dir>/outputs` for existing users. Resolution-dependent placement: inline in the toolbar next to the search bar on a maximized window, inside `HistoryFilterDrawer` (icon changed `ListFilter` → `Settings`, since that surface now mixes filters and configuration) on a minimized one — the permanent filter rail stays filter-only. See `spec-backend.md` § "Script Run History" / "Script Run History — Disk Format".
- Script run-history **write side** (2026-06-25): `script_log_write` (`script_log_service.rs`) is wired into `use-script-remote.ts`'s `executeScript` — once `runRemoteScript` resolves with an exit code, it fire-and-forget writes a real `.json` into `logsDirectoryPath` (status derived from `exit_code`, `triggered_by` resolved from the OS session, both in Rust). `runRemoteScript` (`use-terminal-store.ts`) now also captures the **raw** `pty:data` text between sending the run command and the OSC `DM-DONE` marker (not xterm's rendered buffer) so real ANSI codes survive into the log. `HistoryView` refreshes its list on mount so a just-run script shows up without restarting the app. See `spec-backend.md` § "Script Run History" / "Script Run History — Disk Format".
- Historial's detail log viewer now renders through a dedicated read-only `xterm.js` instance (`history-log-terminal.tsx`), replacing the old heuristic `classifyLine` line classifier — necessary since the captured `output` can contain real ANSI/SGR codes that classifier could never interpret.

**Next in queue:**
1. Settings screen
2. Monitoring snapshot persistence — append-only JSONL day-files under `app_data_dir()/monitoring/`, to back the range tabs (30min/1h/6h/24h) in `monitor.tsx`, currently a "not implemented" stub — see `spec-backend.md` § "Monitoring Snapshots"
